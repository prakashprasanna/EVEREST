...
$apiurl= "https://app.maplecrm.cloud/services/wsapi/JSON/dispatcher_v1/";

$jsondata = array();

$jsondata['FNAME']=$request->input('firstname');

$jsondata['EMAIL']=$request->input('gef_email');

$jsondata['PHONE']=$request->input('gef_phone');

$jsondata['LOCATION']=$request->input('location');

$jsondata['NATIONALITY']=$request->input('nationality');

$jsondata['SOURCE']=$request->input('source');

$jsondata['DESTINATION']=$request->input('destination');

$jsondata['SKYPE']=$request->input('skype');

$jsondata['MESSAGE']=$request->input('message');

$jsondata['PATHWAY']=$request->input('pathway');

 

$jsondata['apitoken']='95b2c553179c50a1'; 
...
if($response != '') {
   ...
   $rarray = json_decode($response,1); //$rarray shall contain 2 fields RETURN (TRUE/FALSE) and MESSAGE
  ...
  public function updateAssignTo(Request $request, $gef_id)
    {
      ...
      if($emp->AJV_DEP_ID === '1'){
        ...
        if($lead != null){
          if($request->input('status') === 'Drop Pending'){ 
             ...
             $gef->save();
                ...
                $gefDEL = gef_DL::where('gef_phone', '=', $gef_id)->first();
                if($gefDEL === null){
       ...
       if($request->input('assignTo') != null){
        $gefnl = enquiry::where('gef_phone', '=', $gef_id)->first();
        if($gefnl === null){
    ...
    public function leadView(Request $request,$gef_phone)
{
    ...
    if($gef->gef_salesApproval === 'Approval Pending'){
    ...
    if($gonogo === null){
       $gonogo = new gonogo;   
       $gonogo->gef_phone = $gef_phone; 
       $gonogo->save();
       $english = new english;   
       $english->gef_phone = $gef_phone;   
       $english->save();
       $academics = new academics;   
       $academics->gef_phone = $gef_phone;   
       $academics->save();
       $work = new work;   
       $work->gef_phone = $gef_phone;   
       $work->save();
       $finance = new finance;   
       $finance->gef_phone = $gef_phone;   
       $finance->save();
       $outcome = new outcome;     
       $outcome->gef_phone = $gef_phone;   
       $outcome->save();  
    }
			...
			if ($extension == "xlsx" || $extension == "xls" || $extension == "csv") {
				...
				if(!empty($data)){
					...
					foreach ($data as $key => $value) { 
                        ...
                        $uploadCount++;
                              if($newgonogo === null){
                                    $newgef = enquiry::where('gef_phone','=',$value->phone)->first();
                                    $gonogo = new gonogo;   
            						$gonogo->gef_phone = $value->phone; 
            						$gonogo->gef_id = $newgef->gef_id; 
                                    $gonogo->gonogo_skilled = $value->status;
            						$gonogo->save();
            						$english = new english;   
            						$english->gef_phone = $value->phone;   
            						$english->gef_id = $newgef->gef_id;   
            						$english->save();
            						$academics = new academics;   
            						$academics->gef_phone = $value->phone;   
            						$academics->gef_id = $newgef->gef_id;   
            						$academics->save();
            						$work = new work;   
            						$work->gef_phone = $value->phone;   
            						$work->gef_id = $newgef->gef_id;   
            						$work->save();
            						$finance = new finance;   
            						$finance->gef_phone = $value->phone; 
            						$finance->gef_id = $newgef->gef_id;   
            						$finance->save();
            						$outcome = new outcome;     
            						$outcome->gef_phone = $value->phone;   
            						$outcome->gef_id = $newgef->gef_id;   
            						$outcome->save();  
                            }                           
                                        }
                                      }
